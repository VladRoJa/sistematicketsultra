#   backend\app\warehouse\services\cargos_recurrentes_repository.py
...
class CargosRecurrentesRepositoryError(RuntimeError):
    """Error base del repository estructurado de Cargos Recurrentes."""


def register_cargos_recurrentes_repository(app) -> None:
    app.config["WAREHOUSE_CARGOS_RECURRENTES_REPOSITORY"] = persist_cargos_recurrentes_snapshot
        ...
        normalized_rows.append(
            {
                ...
                "fecha_proximo_pago": _normalize_required_text(
                    ...
                    field_name="fecha_proximo_pago",
                ),
                "numero_intentos": _ensure_int(
                    row.get("numero_intentos"),
                    field_name="numero_intentos",
                ),
                "hd": _normalize_optional_text(row.get("hd")),
                "estatus": _normalize_required_text(
                    row.get("estatus"),
                    field_name="estatus",
                ),
                "importe": _ensure_decimal(
                    row.get("importe"),
                    field_name="importe",
                ),
                "meses_pendiente": _normalize_optional_text(row.get("meses_pendiente")),
                "fecha_fin_contrato": _normalize_optional_text(row.get("fecha_fin_contrato")),
                "tipo_contrato": _normalize_optional_text(row.get("tipo_contrato")),
                "contrato_ajuste": _normalize_optional_text(row.get("contrato_ajuste")),
...
) -> CargosRecurrentesSnapshotORM | None:
    return (
        CargosRecurrentesSnapshotORM.query.filter_by(
...
def _fetch_existing_canonical_snapshot_for_day(
    *,
    ...
    return (
        ...
        .order_by(CargosRecurrentesSnapshotORM.id.desc())
        .first()
    )
...
def _insert_snapshot_header(
    *,
    ...
    snapshot = CargosRecurrentesSnapshotORM(
        warehouse_upload_id=warehouse_upload_id,
    ...
    *,
    ...
    orm_rows = [
        CargosRecurrentesSnapshotRowORM(
            ...
            fecha_proximo_pago=row["fecha_proximo_pago"],
            numero_intentos=row["numero_intentos"],
            hd=row["hd"],
            estatus=row["estatus"],
            importe=row["importe"],
            meses_pendiente=row["meses_pendiente"],
            fecha_fin_contrato=row["fecha_fin_contrato"],
            tipo_contrato=row["tipo_contrato"],
            contrato_ajuste=row["contrato_ajuste"],
...
def _clear_existing_canonical_snapshot(
    *,
    existing_snapshot: CargosRecurrentesSnapshotORM | None,
) -> None:
...
def persist_cargos_recurrentes_snapshot(
    *,
    warehouse_upload_id: int,
    report_type_key: str,
    ...
    if report_type_key != CARGOS_RECURRENTES_REPORT_TYPE_KEY:
        raise CargosRecurrentesRepositoryError(
            f"report_type_key inválido para Cargos Recurrentes: {report_type_key!r}"
        )
    ...
    if not normalized_rows:
        raise CargosRecurrentesRepositoryError(
            "No se puede persistir un snapshot Cargos Recurrentes sin rows válidas."
        )
    ...
    try:
        ...
        raise CargosRecurrentesRepositoryError(
            "Falló la persistencia estructurada de Cargos Recurrentes por conflicto de integridad."
        ) from exc

    except Exception as exc:
        db.session.rollback()
        if isinstance(exc, CargosRecurrentesRepositoryError):
            raise
        raise CargosRecurrentesRepositoryError(
            "Falló la persistencia estructurada de Cargos Recurrentes."
        ) from exc