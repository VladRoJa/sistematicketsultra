#   backend/app/warehouse/services/corte_caja_repository.py
...
class CorteCajaRepositoryError(RuntimeError):
    """Error base del repository estructurado de Corte de Caja."""


def register_corte_caja_repository(app) -> None:
    app.config["WAREHOUSE_CORTE_CAJA_REPOSITORY"] = persist_corte_caja_snapshot
        ...
        normalized_rows.append(
            {
                ...
                "pago": _normalize_optional_text(row.get("pago")),
                "renovacion": _normalize_optional_text(row.get("renovacion")),
                "operacion": _normalize_optional_text(row.get("operacion")),
                "tipo_pago": _normalize_optional_text(row.get("tipo_pago")),
                "recepcion": _normalize_optional_text(row.get("recepcion")),
...
) -> CorteCajaSnapshotORM | None:
    return (
        CorteCajaSnapshotORM.query.filter_by(
...
def _fetch_existing_canonical_snapshot_for_day(
    *,
    ...
    return (
        ...
        .order_by(CorteCajaSnapshotORM.id.desc())
        .first()
    )
...
def _clear_existing_canonical_snapshot(
    *,
    existing_snapshot: CorteCajaSnapshotORM | None,
) -> None:
    ...
    *,
    ...
    if report_type_key != CORTE_CAJA_REPORT_TYPE_KEY:
        raise CorteCajaRepositoryError(
            f"report_type_key inválido para Corte de Caja: {report_type_key!r}"
        )
    ...
    if not normalized_rows:
        raise CorteCajaRepositoryError(
            "No se puede persistir un snapshot Corte de Caja sin rows válidas."
        )
    ...
    try:
        ...
        raise CorteCajaRepositoryError(
            "Falló la persistencia estructurada de Corte de Caja por conflicto de integridad."
        ) from exc

    except Exception as exc:
        db.session.rollback()
        if isinstance(exc, CorteCajaRepositoryError):
            raise
        raise CorteCajaRepositoryError(
