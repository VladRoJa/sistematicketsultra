#  backend\app\warehouse\services\venta_total_ingestion_service.py
...
class VentaTotalIngestionError(RuntimeError):
    """Error base de la ingesta estructurada de Venta Total."""


class VentaTotalUploadLoadError(VentaTotalIngestionError):
    """Fallo al cargar el upload documental desde Warehouse."""


class VentaTotalParseInvocationError(VentaTotalIngestionError):
    """Fallo al parsear el upload de Venta Total."""


class VentaTotalPersistError(VentaTotalIngestionError):
    """Fallo al persistir el snapshot estructurado de Venta Total."""


@dataclass(slots=True)
class IngestVentaTotalCommand:
...
class WarehouseUploadDocument:
    ...
    def validate(self) -> None:
        if self.report_type_key != VENTA_TOTAL_REPORT_TYPE_KEY:
            raise VentaTotalUploadLoadError(
                "El upload recibido no corresponde a 'venta_total'. "
                f"Recibido: {self.report_type_key!r}"
            )

        if not self.file_path and self.file_bytes is None:
            raise VentaTotalUploadLoadError(
                "El upload Venta Total debe traer 'file_path' o 'file_bytes'."
            )

        if self.date_to is None and self.cutoff_date is None:
            raise VentaTotalUploadLoadError(
                "El upload Venta Total debe traer 'date_to' o 'cutoff_date' resuelto."
            )


@dataclass(slots=True)
class IngestVentaTotalResult:
    status: str
    was_idempotent: bool
    snapshot_id: int
    warehouse_upload_id: int
...
def register_venta_total_ingestor(app) -> None:
    app.config["WAREHOUSE_VENTA_TOTAL_INGESTOR"] = ingest_venta_total_upload
...
def _as_dict(value: Any) -> dict[str, Any]:
    ...
    raise VentaTotalIngestionError(
...
def _validate_command(command: IngestVentaTotalCommand) -> None:
    ...
    if command.snapshot_kind not in SUPPORTED_SNAPSHOT_KINDS:
        raise VentaTotalIngestionError(
            f"snapshot_kind inválido para Venta Total: {command.snapshot_kind!r}. "
...
def _resolve_parser() -> Callable[..., Any]:
    return _resolve_callable(
        config_keys=("WAREHOUSE_VENTA_TOTAL_PARSER",),
        description="parsear el XLSX de Venta Total",
        default=parse_venta_total_xlsx,
    )


def _resolve_repository() -> Callable[..., Any]:
    return _resolve_callable(
        config_keys=("WAREHOUSE_VENTA_TOTAL_REPOSITORY",),
        description="persistir el snapshot estructurado de Venta Total",
        default=persist_venta_total_snapshot,
    )


def _resolve_optional_canonicality_resolver() -> Callable[..., dict[str, Any] | None] | None:
    fn = current_app.config.get("WAREHOUSE_VENTA_TOTAL_CANONICALITY_RESOLVER")
...
def _resolve_optional_advisory_lock_key(
    *,
    ...
    resolver = current_app.config.get("WAREHOUSE_VENTA_TOTAL_ADVISORY_LOCK_RESOLVER")
...
def _load_upload_document(
    *,
    ...
    try:
        ...
        raise VentaTotalUploadLoadError(
            f"Falló la carga del upload documental Venta Total para warehouse_upload_id={warehouse_upload_id}."
        ) from exc
...
def _normalize_parsed_snapshot(raw_result: Any) -> dict[str, Any]:
    if isinstance(raw_result, VentaTotalParseResult):
    ...
    if not isinstance(rows, list) or not rows:
        raise VentaTotalParseInvocationError(
            "El parser de Venta Total no devolvió rows válidas."
        )
...
def _parse_upload_document(
    *,
    ...
    try:
        ...
        raise VentaTotalParseInvocationError(
            f"Falló el parser de Venta Total para warehouse_upload_id={upload_doc.warehouse_upload_id}."
        ) from exc
...
def _resolve_business_date(upload_doc: WarehouseUploadDocument) -> date:
    ...
    raise VentaTotalUploadLoadError(
        "No se pudo resolver business_date para Venta Total."
    )


def _build_result(repository_result: Any) -> IngestVentaTotalResult:
    payload = _as_dict(repository_result)

    return IngestVentaTotalResult(
...
def ingest_venta_total_upload(
    *,
    ...
    try:
        ...
        raise VentaTotalPersistError(
            f"Falló la persistencia estructurada de Venta Total para warehouse_upload_id={upload_doc.warehouse_upload_id}."
        ) from exc
