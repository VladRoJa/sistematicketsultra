# backend/app/warehouse/services/ingresos_wellhub_repository.py
...
SNAPSHOTS_TABLE = "ingresos_wellhub_snapshots"
ROWS_TABLE = "ingresos_wellhub_snapshot_rows"


def register_ingresos_wellhub_repository(app) -> None:
    app.config["WAREHOUSE_INGRESOS_WELLHUB_REPOSITORY"] = (
        persist_ingresos_wellhub_snapshot
    )
        ...
        params.append(
            {
                ...
                "visitor_name": str(row.get("visitor_name") or "").strip() or None,
                "wellhub_member_id": str(row["wellhub_member_id"]).strip(),
                "total_checkins_mtd": _to_db_int(row.get("total_checkins_mtd")),
                "pago_total_mtd": _to_db_numeric(row.get("pago_total_mtd")),
            }
        )
...
def persist_ingresos_wellhub_snapshot(
    *,
    parsed_snapshot: Any,
    snapshot_kind: str,
    ...
    """
    Persistencia transaccional de ingresos_wellhub.
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Revisa constraints/índices de ingresos_wellhub."
        ) from exc
    except Exception:
        session.rollback()
        raise