#   backend/app/warehouse/services/corte_caja_ingestion_service.py
...
from app.warehouse.services.corte_caja_parser import (
    CorteCajaParseResult,
    parse_corte_caja_xlsx,
)
from app.warehouse.services.corte_caja_repository import (
    CORTE_CAJA_REPORT_TYPE_KEY,
    persist_corte_caja_snapshot,
)
...
class CorteCajaIngestionError(RuntimeError):
    """Error base de la ingesta estructurada de Corte de Caja."""


class CorteCajaUploadLoadError(CorteCajaIngestionError):
    """Fallo al cargar el upload documental desde Warehouse."""


class CorteCajaParseInvocationError(CorteCajaIngestionError):
    """Fallo al parsear el upload de Corte de Caja."""


class CorteCajaPersistError(CorteCajaIngestionError):
    """Fallo al persistir el snapshot estructurado de Corte de Caja."""


@dataclass(slots=True)
class IngestCorteCajaCommand:
...
class WarehouseUploadDocument:
    ...
    def validate(self) -> None:
        if self.report_type_key != CORTE_CAJA_REPORT_TYPE_KEY:
            raise CorteCajaUploadLoadError(
                "El upload recibido no corresponde a 'corte_caja'. "
                f"Recibido: {self.report_type_key!r}"
            )

        if not self.file_path and self.file_bytes is None:
            raise CorteCajaUploadLoadError(
                "El upload Corte de Caja debe traer 'file_path' o 'file_bytes'."
            )

        # Contractualmente, Corte de Caja debe traer rango documental.
        if self.date_to is None and self.cutoff_date is None:
            raise CorteCajaUploadLoadError(
                "El upload Corte de Caja debe traer 'date_to' o 'cutoff_date' resuelto."
            )


@dataclass(slots=True)
class IngestCorteCajaResult:
    status: str
    was_idempotent: bool
    snapshot_id: int
    warehouse_upload_id: int
...
def register_corte_caja_ingestor(app) -> None:
    app.config["WAREHOUSE_CORTE_CAJA_INGESTOR"] = ingest_corte_caja_upload
...
def _validate_command(command: IngestCorteCajaCommand) -> None:
    ...
    if command.snapshot_kind not in SUPPORTED_SNAPSHOT_KINDS:
        raise CorteCajaIngestionError(
            f"snapshot_kind inválido para Corte de Caja: {command.snapshot_kind!r}. "
...
def _resolve_parser() -> Callable[..., Any]:
    return _resolve_callable(
        config_keys=("WAREHOUSE_CORTE_CAJA_PARSER",),
        description="parsear el XLSX de Corte de Caja",
        default=parse_corte_caja_xlsx,
    )


def _resolve_repository() -> Callable[..., Any]:
    return _resolve_callable(
        config_keys=("WAREHOUSE_CORTE_CAJA_REPOSITORY",),
        description="persistir el snapshot estructurado de Corte de Caja",
        default=persist_corte_caja_snapshot,
    )


def _resolve_optional_canonicality_resolver() -> Callable[..., dict[str, Any] | None] | None:
    fn = current_app.config.get("WAREHOUSE_CORTE_CAJA_CANONICALITY_RESOLVER")
...
def _resolve_optional_advisory_lock_key(
    *,
    ...
    resolver = current_app.config.get("WAREHOUSE_CORTE_CAJA_ADVISORY_LOCK_RESOLVER")
...
def _load_upload_document(
    *,
    ...
    try:
        ...
        raise CorteCajaUploadLoadError(
            f"Falló la carga del upload documental Corte de Caja para warehouse_upload_id={warehouse_upload_id}."
        ) from exc
...
def _normalize_parsed_snapshot(raw_result: Any) -> dict[str, Any]:
    if isinstance(raw_result, CorteCajaParseResult):
    ...
    if not isinstance(rows, list) or not rows:
        raise CorteCajaParseInvocationError(
            "El parser de Corte de Caja no devolvió rows válidas."
        )
...
def _parse_upload_document(
    *,
    ...
    try:
        ...
        raise CorteCajaParseInvocationError(
            f"Falló el parser de Corte de Caja para warehouse_upload_id={upload_doc.warehouse_upload_id}."
        ) from exc
...
def _resolve_business_date(upload_doc: WarehouseUploadDocument) -> date:
    ...
    raise CorteCajaUploadLoadError(
        "No se pudo resolver business_date para Corte de Caja."
    )


def _build_result(repository_result: Any) -> IngestCorteCajaResult:
    payload = _as_dict(repository_result)

    return IngestCorteCajaResult(
...
def ingest_corte_caja_upload(
    *,
    ...
    command = IngestCorteCajaCommand(
        warehouse_upload_id=warehouse_upload_id,
        snapshot_kind=snapshot_kind,
    ...
    try:
        ...
        raise CorteCajaPersistError(
            f"Falló la persistencia estructurada de Corte de Caja para warehouse_upload_id={upload_doc.warehouse_upload_id}."
        ) from exc
