#  backend/app/warehouse/services/kpi_ventas_nuevos_socios_ingestion_service.py
...
from app.warehouse.services.kpi_ventas_nuevos_socios_parser import (
    KpiVentasNuevosSociosParseResult,
    parse_kpi_ventas_nuevos_socios_xlsx,
)
from app.warehouse.services.kpi_ventas_nuevos_socios_repository import (
    KPI_VENTAS_NUEVOS_SOCIOS_REPORT_TYPE_KEY,
...
class KpiVentasNuevosSociosIngestionError(RuntimeError):
    """Error base de la ingesta estructurada de KPI Ventas Nuevos Socios."""


class KpiVentasNuevosSociosUploadLoadError(KpiVentasNuevosSociosIngestionError):
    """Fallo al cargar el upload documental desde Warehouse."""


class KpiVentasNuevosSociosParseInvocationError(KpiVentasNuevosSociosIngestionError):
    """Fallo al parsear el upload KPI Ventas Nuevos Socios."""


class KpiVentasNuevosSociosPersistError(KpiVentasNuevosSociosIngestionError):
    """Fallo al persistir el snapshot estructurado KPI Ventas Nuevos Socios."""


@dataclass(slots=True)
class IngestKpiVentasNuevosSociosCommand:
...
class WarehouseUploadDocument:
    ...
    def validate(self) -> None:
        if self.report_type_key != KPI_VENTAS_NUEVOS_SOCIOS_REPORT_TYPE_KEY:
            raise KpiVentasNuevosSociosUploadLoadError(
                "El upload recibido no corresponde a 'kpi_ventas_nuevos_socios'. "
                f"Recibido: {self.report_type_key!r}"
            )

        if not self.file_path and self.file_bytes is None:
            raise KpiVentasNuevosSociosUploadLoadError(
                "El upload KPI Ventas Nuevos Socios debe traer 'file_path' o 'file_bytes'."
            )

        if self.cutoff_date is None:
            raise KpiVentasNuevosSociosUploadLoadError(
                "El upload KPI Ventas Nuevos Socios debe traer 'cutoff_date' resuelto."
            )


@dataclass(slots=True)
class IngestKpiVentasNuevosSociosResult:
    status: str
    was_idempotent: bool
    snapshot_id: int
    warehouse_upload_id: int
...
def register_kpi_ventas_nuevos_socios_ingestor(app) -> None:
    app.config["WAREHOUSE_KPI_VENTAS_NUEVOS_SOCIOS_INGESTOR"] = (
...
def _as_dict(value: Any) -> dict[str, Any]:
    ...
    raise KpiVentasNuevosSociosIngestionError(
...
def _validate_command(command: IngestKpiVentasNuevosSociosCommand) -> None:
    ...
    if command.snapshot_kind not in SUPPORTED_SNAPSHOT_KINDS:
        raise KpiVentasNuevosSociosIngestionError(
            f"snapshot_kind inválido para KPI Ventas Nuevos Socios: {command.snapshot_kind!r}. "
...
def _resolve_parser() -> Callable[..., Any]:
    return _resolve_callable(
        config_keys=("WAREHOUSE_KPI_VENTAS_NUEVOS_SOCIOS_PARSER",),
        description="parsear el XLSX de KPI Ventas Nuevos Socios",
        default=parse_kpi_ventas_nuevos_socios_xlsx,
    )


def _resolve_repository() -> Callable[..., Any]:
    return _resolve_callable(
        config_keys=("WAREHOUSE_KPI_VENTAS_NUEVOS_SOCIOS_REPOSITORY",),
        description="persistir el snapshot estructurado de KPI Ventas Nuevos Socios",
        default=persist_kpi_ventas_nuevos_socios_snapshot,
    )


def _resolve_optional_canonicality_resolver() -> Callable[..., dict[str, Any] | None] | None:
    fn = current_app.config.get(
        "WAREHOUSE_KPI_VENTAS_NUEVOS_SOCIOS_CANONICALITY_RESOLVER"
    )
...
def _resolve_optional_advisory_lock_key(
    *,
    ...
    resolver = current_app.config.get(
        "WAREHOUSE_KPI_VENTAS_NUEVOS_SOCIOS_ADVISORY_LOCK_RESOLVER"
    )
...
def _normalize_parsed_snapshot(raw_result: Any) -> dict[str, Any]:
    if isinstance(raw_result, KpiVentasNuevosSociosParseResult):
    ...
    if not isinstance(rows, list) or not rows:
        raise KpiVentasNuevosSociosParseInvocationError(
            "El parser de KPI Ventas Nuevos Socios no devolvió rows válidas."
        )
...
def _parse_upload_document(
    *,
    ...
    try:
        ...
        raise KpiVentasNuevosSociosParseInvocationError(
            f"Falló el parser de KPI Ventas Nuevos Socios para warehouse_upload_id={upload_doc.warehouse_upload_id}."
        ) from exc

    return _normalize_parsed_snapshot(raw_result)


def _build_result(repository_result: Any) -> IngestKpiVentasNuevosSociosResult:
    payload = _as_dict(repository_result)

    return IngestKpiVentasNuevosSociosResult(
...
def ingest_kpi_ventas_nuevos_socios_upload(
    *,
    warehouse_upload_id: int,
    snapshot_kind: str,
    requested_by: str | None = None,
    ...
    command = IngestKpiVentasNuevosSociosCommand(
        warehouse_upload_id=warehouse_upload_id,
        snapshot_kind=snapshot_kind,
    ...
    try:
        ...
        raise KpiVentasNuevosSociosPersistError(
            f"Falló la persistencia estructurada de KPI Ventas Nuevos Socios para warehouse_upload_id={upload_doc.warehouse_upload_id}."
        ) from exc
