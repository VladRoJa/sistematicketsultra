...
def test_appointment_close_rules_close_case_except_no_show():
    ...
    assert "case.closed_at = now" in service
    assert "case.closed_by_user_id = actor.id" in service


def test_purchase_reported_keeps_purchase_verification_flow():
    service = _read(SERVICE)
