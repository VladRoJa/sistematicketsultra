...
from app.models.warehouse import VentaTotalSnapshotORM
from app.warehouse.services import venta_total_repository as repository
...
def isolated_app():
    ...
    with app.app_context():
        db.session.execute(
            text(
                """
                CREATE TABLE venta_total_snapshots (
...
def test_manual_promotion_accepts_corrected_snapshot_with_fewer_rows(
    isolated_app,
):
    ...
    with isolated_app.app_context():
        ...
        assert corrected_snapshot.row_count_valid < (
            previous_snapshot.row_count_valid
        )

        result = repository.promote_venta_total_snapshot_canonical(
...
def test_manual_promotion_rejects_unexpected_business_date(
    isolated_app,
):
    actual_date = date(2026, 7, 31)
    ...
    with isolated_app.app_context():
        ...
        with pytest.raises(
            repository.VentaTotalRepositoryError,
            match="business_date inesperado",
        ):
            repository.promote_venta_total_snapshot_canonical(
                snapshot_id=snapshot.id,
                expected_business_date=date(2026, 8, 1),
                expected_snapshot_kind="daily",
                auto_commit=True,
            )
